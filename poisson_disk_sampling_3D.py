...
class Grid:
    ...
    def generate_point(self):
        list = []
        list.append(random.uniform(0, self.length))
        list.append(random.uniform(0, self.width))
        list.append(random.uniform(0, self.height))
        return list

    def getcoordinate(self, point):
        list = []
        for i in range(self.dimension):
            list.append(math.floor(point[i] / self.cellsize))
        return list
    ...
    def inGrid(self, point):
        if point[0] < 0 or point[0] > self.length:
            return False
